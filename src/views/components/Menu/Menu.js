...
const Menu = ({week}) => {
  ...
  const { loading, success, error, days } = useSelector(state => state.dayList);
        ...
        return (
          <Paper key={`day-card-${dayIdx}`} className={classes.root}>
            <div className={classes.header}>
              <div>{day.dayName}</div>
              <IconButton className={classes.action} onClick={() => history.push("/days/edit")} >
                <EditIcon />
              </IconButton>
            </div>
            <div className={classes.content}>
              {day.meals.map((meal, mealIdx) => {
                return (
                  <div key={`meal-in-day-card-${mealIdx}`} className={classes.menuMeal}>
